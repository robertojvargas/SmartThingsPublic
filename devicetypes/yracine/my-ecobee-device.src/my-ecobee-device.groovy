...
*/        
		...
		standardTile("switchProgram", "device.programNameForUI",  canChangeIcon: false,
			...
			state "Sleep", label: 'Set:${name}', action: "away", 
				...
				nextState: "Away"
			state "Away", label: 'Set:${name}', action: "quickSave", 
				icon: "st.presence.car.car",backgroundColor: "#ffffff",
				nextState: "Save"
			state "Save", label: 'Set:${name}', action: "home", 
...
private void updateCurrentTileValue() {
	...
	if (state?.scale =='C') {    
		...
		if (tempValueString.matches(".*([.,][456])")) {                
			...
			traceEvent(settings.logFilter,"updateCurrentTileValue>value $tempValueString which ends with 456=> rounded to .5", settings.trace)	
			...
			traceEvent(settings.logFilter,"updateCurrentTileValue>value $tempValueString which ends with 0123=> rounded to previous .0", settings.trace)	
			tempValueString=String.format('%2d.0', state?.currentTileValue.intValue())               
		}
...
// value is in Farenheit && must me a multiple of 5 between 0..50.
void setDehumidifyACOffset(value) { 
	if ((!value.isInteger()) || (value <0 || value > 50)) {
...
*/    
	...
	heatAuxRuntimeDaily= ((dailyHeatingRuntimeAux1?:0) + (dailyHeatingRuntimeAux2?:0) + (dailyHeatingRuntimeAux3?:0)).toFloat().round(1)
	heatAuxRuntimeYesterday=((yesterdayHeatingRuntimeAux1?:0) + (yesterdayHeatingRuntimeAux2?:0) + (yesterdayHeatingRuntimeAux3?:0)).toFloat().round(1)
	heatAuxRuntimeAvgWeekly=((avgWeeklyHeatingRuntimeAux1?:0) + (avgWeeklyHeatingRuntimeAux2?:0) + (avgWeeklyHeatingRuntimeAux3?:0)).toFloat().round(1)
	heatAuxRuntimeAvgMonthly=((avgMonthlyHeatingRuntimeAux1?:0) + (avgMonthlyHeatingRuntimeAux2?:0) + (avgMonthlyHeatingRuntimeAux3?:0)).toFloat().round(1)
	if (settings.trace) {
	...
	if (heatStages>1) { 
		...
		ratioAuxWeekly=0
		ratioAuxMonthly=0        
		if (dailyHeatingRuntimeComp1) {
			ratioAuxDaily=((dailyHeatingRuntimeComp1/dailyHeatingRuntime)*100).round(1)
		}            
		traceEvent(settings.logFilter,"get_tips_level4>ratioHeatComp1Daily=$ratioAuxDaily %",settings.trace)
		if (yesterdayHeatingRuntimeComp1) {
			ratioAuxYesterday=((yesterdayHeatingRuntimeComp1/yesterdayHeatingRuntime)*100).round(1)
		}            
		traceEvent(settings.logFilter,"get_tips_level4>ratioHeatComp1Yesterday=$ratioAuxYesterday %",settings.trace)
		if (avgWeeklyHeatingRuntimeComp1) {
			ratioAuxWeekly=((avgWeeklyHeatingRuntimeComp1/avgWeeklyHeatingRuntime)*100).round(1)
		}            
		traceEvent(settings.logFilter,"get_tips_level4>ratioHeatComp1Weekly=$ratioAuxWeekly %",settings.trace)
		if (avgWeeklyHeatingRuntimeComp1) {
			ratioAuxMonthly=((avgWeeklyHeatingRuntimeComp1/avgMonthlyHeatingRuntime)*100).round(1)
		}            
		...
		if ((state?.currentTip < maxTips) && (!tipsAlreadyGiven.contains("tip40")) &&  (dailyHeatingRuntime) && (((avgOutdoorTemperature > LOW_TEMP_THRESHOLD_FOR_HEATING)) &&
			...
			sendEvent name: attribute, value: "Observation: The average outdoor temperature in the past week has been ${avgOutdoorTemperature} degrees with a standard deviation of ${devOutdoorTemperature} degrees." +
				"You have a ${heatStages}-stage heating system and yesterday's ${component} ratio of ${ratioAuxDaily}% usage versus the Total Heating Runtime is lower than ${RATIO_MIN_AUX_HEAT}%." +  
				"Yesterday's ${component} Runtime is ${dailyCoolingRuntimeComp1} versus the total Heating Runtime of ${dailyHeatingRuntime} minutes." +  
				"Current tip is: " + recommendation.text,displayed: (settings.trace?:false)
			attribute ="tip${state?.currentTip}Level"                
			sendEvent name: attribute, value: recommendation.level,displayed: (settings.trace?:false)
			attribute ="tip${state?.currentTip}Solution"                
		...
		if ((state?.currentTip < maxTips) && (!tipsAlreadyGiven.contains("tip45")) && (yesterdayHeatingRuntime) && (ratioAuxDaily < ratioAuxYesterday)) {            
			...
			sendEvent name: attribute, value: "Observation: The average outdoor temperature in the past week has been ${avgOutdoorTemperature} degrees with a standard deviation of ${devOutdoorTemperature} degrees." +
				"You have a ${heatStages}-stage heating system and yesterday's ${component} ratio of ${ratioAuxDaily}% usage versus the total Heating Runtime is lower than the day before's ratio of ${ratioAuxYesterday}%." +  
				"Yesterday's ${component} Runtime is ${dailyHeatingRuntimeComp1} versus the day before's ${component} Runtime of ${yesterdayHeatingRuntimeComp1} minutes." +  
				"Current tip is: " + recommendation.text,displayed: (settings.trace?:false)
			attribute ="tip${state?.currentTip}Level"                
			sendEvent name: attribute, value: recommendation.level,displayed: (settings.trace?:false)
			attribute ="tip${state?.currentTip}Solution"                
		...
		if ((state?.currentTip < maxTips) && (!tipsAlreadyGiven.contains("tip46")) &&  (avgWeeklyHeatingRuntime) && (ratioAuxDaily < ratioAuxWeekly)) {            
			...
			sendEvent name: attribute, value: "Observation: The average outdoor temperature in the past week has been ${avgOutdoorTemperature} degrees with a standard deviation of ${devOutdoorTemperature} degrees." +
				...
				"Yesterday's ${component}Runtime is ${dailyHeatingRuntimeComp1} versus the weekly Average ${component} Runtime of ${c} minutes." +  
				"Current tip is: " + recommendation.text,displayed: (settings.trace?:false)
			attribute ="tip${state?.currentTip}Level"                
			sendEvent name: attribute, value: recommendation.level,displayed: (settings.trace?:false)
			attribute ="tip${state?.currentTip}Solution"                
		...
		if ((state?.currentTip < maxTips) && (!tipsAlreadyGiven.contains("tip47")) &&  (avgMonthlyHeatingRuntime) && (ratioAuxDaily < ratioAuxMonthly)) {            
			...
			sendEvent name: attribute, value: "Observation: The average outdoor temperature in the past week has been ${avgOutdoorTemperature} degrees with a standard deviation of ${devOutdoorTemperature} degrees." +
				"You have a ${heatStages}-stage heating system and yesterday's ${component} ratio of ${ratioAuxDaily}% usage versus the total Heating Runtime is lower than the monthly average ratio of ${ratioAuxMonthly}%." +  
				"Yesterday's ${component} Runtime is ${dailyHeatingRuntimeComp1} versus the monthly average ${component} Runtime of ${avgMonthlyHeatingRuntimeComp1} minutes." +  
				"Current tip is: " + recommendation.text,displayed: (settings.trace?:false)
			attribute ="tip${state?.currentTip}Level"                
			sendEvent name: attribute, value: recommendation.level, displayed: (settings.trace?:false)
			attribute ="tip${state?.currentTip}Solution"                
